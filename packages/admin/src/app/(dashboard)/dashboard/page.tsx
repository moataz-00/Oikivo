...
function StatCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 hover:border-gray-700 transition-colors group">
...
function ActionCard({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center justify-between rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-5 py-4 hover:border-indigo-600/50 hover:bg-gray-800/60 transition-all"
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-bold text-gray-900 dark:text-white text-xl">{count}</p>
            ...
            return (
              <div key={d.month} className="group flex-1 flex flex-col items-center justify-end h-full gap-1">
                <div className="relative w-full flex items-end justify-end" style={{ height: '100%' }}>
                  ...
                  />
                  <div className="absolute bottom-full mb-1.5 left-1/2 -translate-x-1/2 hidden group-hover:flex flex-col items-center z-10 whitespace-nowrap rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 px-3 py-2 text-xs text-gray-900 dark:text-white shadow-xl">
...
export default function DashboardPage() {
  ...
  const { data: chart } = useQuery({
    queryKey: ['admin-revenue-chart', dateRange.from, dateRange.to],
    queryFn: () => adminApi.getRevenueChart(dateRange),
  });
    ...
    return (
      <div className="space-y-6 animate-pulse">
        <div className="flex justify-between">
          ...
          <div className="h-8 bg-gray-100 dark:bg-gray-800 rounded-lg w-24" />
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => <div key={i} className="h-28 bg-gray-100 dark:bg-gray-800 rounded-xl" />)}
        </div>
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center gap-3 py-24 text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">The backend may be unavailable.</p>
        <button onClick={() => refetch()} className="mt-2 rounded-lg bg-gray-100 dark:bg-gray-800 px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="flex items-center gap-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 px-3 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors disabled:opacity-50"
            >
        ...
        {/* Date range filter */}
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-white/60 dark:bg-gray-900/60 px-3 py-2">
          ...
          {activePreset === 'custom' && (
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="rounded-md bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 px-2 py-1 text-xs text-gray-600 dark:text-gray-300 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              />
      ...
      {((s?.pendingActions?.openDisputes ?? 0) > 0 ||
        (s?.pendingActions?.pendingPayouts ?? 0) > 0 ||
        (s?.pendingActions?.pendingIdVerifications ?? 0) > 0 ||
        (s?.pendingActions?.pendingInstapayRefunds ?? 0) > 0) && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            {(s?.pendingActions?.pendingInstapayRefunds ?? 0) > 0 && (
              <ActionCard
                label="InstaPay Refunds"
                count={s!.pendingActions!.pendingInstapayRefunds!}
                icon={Banknote}
                accent="bg-sky-700"
                href="/payments/instapay-refunds"
              />
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          trend={(period?.newUsers ?? 0) > 0 ? { value: `${period!.newUsers} new`, positive: true } : undefined}
        />
        <StatCard
          label="Active Hosts"
        ...
        <StatCard
          ...
          trend={{ value: `EGP ${(s?.revenue?.thisWeek ?? 0).toLocaleString()} this week`, positive: (s?.revenue?.thisWeek ?? 0) > 0 }}
        />
        <StatCard
            ...
            {(s?.recentBookings ?? []).map((b: any) => (
              <div key={b.id} className="flex items-center justify-between gap-2 text-sm py-2 border-b border-gray-800/60 last:border-0">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="h-8 w-8 shrink-0 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-gray-400">
                    {b.guest?.firstName?.[0]}{b.guest?.lastName?.[0]}
                  </div>
                  <div className="min-w-0">
                    <p className="font-medium text-gray-900 dark:text-white truncate text-sm">{b.guest?.firstName} {b.guest?.lastName}</p>
                    <p className="text-xs text-gray-500 truncate">{b.property?.title}</p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-gray-900 dark:text-white font-semibold text-sm">EGP {b.totalAmount?.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{s?.properties?.draft ?? 0}</p>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{s?.bookings?.cancelled ?? 0}</p>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">EGP {(s?.revenue?.thisWeek ?? 0).toLocaleString()}</p>
        </div>
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
          <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{s?.users?.guests ?? 0}</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            >
              ...
              <span className="text-xs font-medium text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:hover:text-white transition-colors">{item.label}</span>
            </Link>
          ))}
        </div>
      </div>
