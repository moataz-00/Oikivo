  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-sm">
        ...
        <span className="text-gray-400">{count.toLocaleString()} <span className="text-gray-600">({p}%)</span></span>
      </div>
    ...
    <div className="rounded-xl bg-gray-800/60 border border-gray-700 p-4 space-y-3">
      ...
      <div className="text-sm space-y-3">
        {entries.map(([method, v]) => (
          <div key={method} className="rounded-lg bg-gray-900/60 border border-gray-700/50 p-3 space-y-1.5">
...
function ExpensesSection() {
  ...
  const createMutation = useMutation({
    mutationFn: () => adminApi.createExpense({ ...form, amount: parseFloat(form.amount) || 0 }),
    onSuccess: () => {
        ...
        <div className="rounded-lg border border-gray-700 bg-gray-800/60 p-4 space-y-3">
          <div className="grid sm:grid-cols-2 gap-3">
            <input
              ...
              className="rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <input
              type="number"
            ...
            <tbody>
              {items.map((e: any) => (
                <tr key={e.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="py-2 pr-4 text-gray-400">{e.category || '—'}</td>
...
export default function AnalyticsPage() {
  ...
  const conversionRate = s?.users?.total > 0 ? ((totalBookings / s.users.total) * 100).toFixed(1) : '0.0';
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Platform performance, financials & profit tracking</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {(fromDate || toDate) && (
            <button
              ...
              className="rounded-lg bg-gray-700 p-2 text-gray-400 hover:text-white transition-colors"
              title="Clear filter"
            >
                ...
                <tbody>
                  {f.hosts.topHosts.map((h: any) => (
                    <tr key={h.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      ...
                      <td className="py-2 pr-4 text-gray-400">{h.email}</td>
                      <td className="py-2 pr-4 text-right text-gray-400">{h.properties}</td>
                      <td className="py-2 pr-4 text-right text-gray-400">{h.bookings}</td>
      ...
      {f && (
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-5 space-y-5">
          ...
          {/* Top Consultants */}
          {f.consultants?.topConsultants?.length > 0 && (
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {f.consultants.topConsultants.map((c: any) => (
                    <tr key={c.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                      ...
                      <td className="py-2 pr-4 text-gray-400">{c.specialization || '—'}</td>
                      <td className="py-2 pr-4 text-right text-gray-400">{c.bookings}</td>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900 p-5 space-y-3">
        <SectionHeader icon={CreditCard} title="Gateway Fee Reference" color="bg-gray-700" sub="Fixed costs from payment providers — not configurable" />
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl bg-gray-800/60 border border-gray-700 p-4 space-y-2">
            ...
            <div className="text-sm space-y-1">
              ...
              <Row label="Min" value="EGP 0.50" valueClass="text-gray-400" />
              <Row label="Max" value="EGP 20" valueClass="text-gray-400" />
              <Row label="Limit per txn" value="EGP 70,000" valueClass="text-gray-400" />
              <Row label="Daily limit/bank" value="EGP 120,000" valueClass="text-gray-400" />
              <Row label="Monthly limit/bank" value="EGP 400,000" valueClass="text-gray-400" />
            </div>
          </div>
        </div>
      </div>
                ...
                <tbody>
                  {(f.monthly.property ?? []).map((m: any) => (
                    <tr key={m.month} className="border-b border-gray-800/50">
                      <td className="py-2 pr-4 text-gray-400">{m.month}</td>
                      <td className="py-2 pr-4 text-right text-white">{m.bookings}</td>
                      <td className="py-2 pr-4 text-right text-white">{egp(m.revenue)}</td>
...
function EnhancedAnalyticsSection() {
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-gray-900 p-5 space-y-4">
      <SectionHeader icon={BarChart3} title="Additional Insights" color="bg-gray-700" />
              ...
              <tbody>
                {e.topProperties.map((p: any) => (
                  <tr key={p.id} className="border-b border-gray-800/50">
                    ...
                    <td className="py-2 pr-4 text-gray-400">{p.city}</td>
                    <td className="py-2 pr-4 text-right text-gray-400">{p.bookings}</td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {e.bookingsByCity.map((c: any) => (
                  <tr key={c.city} className="border-b border-gray-800/50">
                    <td className="py-2 pr-4 text-white">{c.city || 'Unknown'}</td>
                    <td className="py-2 text-right text-gray-400">{c.bookings}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {e.revenueByPaymentMethod?.length > 0 && (
        <div>
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {e.revenueByPaymentMethod.map((m: any) => (
              <div key={m.method} className="rounded-lg bg-gray-800/60 border border-gray-700 p-3">
                <p className="text-xs font-medium text-gray-400 capitalize">{methodLabel(m.method || 'Unknown')}</p>
                <p className="text-lg font-bold text-white mt-1">{egp(m.revenue)}</p>
      ...
      {e.topCancellationReasons?.length > 0 && (
        <div>
