...
async function sendNotificationBlast(payload: {
  audience: AudienceType;
  type: NotifType;
  title: string;
  message: string;
}) {
  return apiClient.post('/admin/notifications/blast', payload).then((r) => r.data);
}

async function getNotificationHistory(params?: { page?: number; limit?: number }) {
...
export default function NotificationsPage() {
  ...
  const historyQuery = useQuery({
    ...
    queryFn: () => getNotificationHistory({ page: 1, limit: 50 }),
    enabled: tab === 'history',
    retry: false,
  });

  const sendMutation = useMutation({
    mutationFn: sendNotificationBlast,
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Send platform announcements and view notification history</p>
      </div>
          ...
          <div className="rounded-xl border border-gray-800 bg-gray-900 p-5 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                maxLength={100}
                className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-indigo-500"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                maxLength={500}
                rows={4}
                className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-indigo-500 resize-none"
              />
            </div>
          ...
          <div className="rounded-xl border border-gray-800 bg-gray-900 p-5">
            ...
            <div className="rounded-xl border border-gray-700 bg-gray-800 p-4 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Journey Stay Admin</p>
                  <p className="text-xs text-gray-600">Just now</p>
                </div>
              </div>
              <p className="text-sm font-semibold text-white">{title || 'Your notification title'}</p>
              <p className="text-sm text-gray-300 leading-relaxed">
                {message || 'Your notification message will appear here...'}
              </p>
            </div>

            <div className="mt-5 rounded-lg bg-gray-800/50 border border-gray-700/50 p-4">
              <p className="text-xs font-medium text-gray-400 mb-2">Sending to:</p>
              <div className="flex items-center gap-2">
                {(() => { const a = AUDIENCE_OPTIONS.find((o) => o.value === audience)!; return <><a.icon className="h-4 w-4 text-indigo-400" /><span className="text-sm text-white">{a.label}</span></>; })()}
              </div>
              <div className="flex items-center gap-2 mt-2">
                <selectedType.icon className={cn('h-4 w-4', selectedType.color.split(' ')[0])} />
                <span className="text-sm text-white">{selectedType.label} notification</span>
              </div>
            </div>
      ...
      {tab === 'history' && (
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-5">
          <h2 className="text-base font-semibold text-white mb-4">Notification History</h2>
          ...
          {historyQuery.isError && (
            <div className="flex flex-col items-center gap-2 py-12 text-center">
              ...
              <p className="text-gray-400 text-sm">Notification history endpoint not yet available.</p>
          ...
          {historyQuery.data && Array.isArray(historyQuery.data) && historyQuery.data.length > 0 && (
            <div className="divide-y divide-gray-800">
              {(historyQuery.data as any[]).map((n: any) => {
                ...
                return (
                  <div key={n.id} className="py-4 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-400 mt-0.5 truncate">{n.message}</p>
                    </div>
