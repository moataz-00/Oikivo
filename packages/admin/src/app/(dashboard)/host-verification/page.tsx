...
export default function HostVerificationPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-users-pending-id'],
    queryFn: () => adminApi.getUsers({ page: 1, limit: 100, role: 'host' }),
  });
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: number) => adminApi.reviewIdDocument(id, false),
    onSuccess: () => {
  ...
  const pendingUsers = users.filter((u: any) => u.idDocument && !u.isIdVerified);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white">Host Verification</h1>
        <p className="text-sm text-gray-400 mt-0.5">Review and approve host ID verification documents</p>
      </div>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-5">
          {!selectedUser ? (
            <div className="flex flex-col items-center gap-2 py-14 text-center">
              ...
              <p className="text-gray-400 text-sm font-medium">Select a host to review</p>
            ...
            <div className="space-y-4">
              ...
              {/* ID Document image */}
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">Submitted ID Document</p>
                {selectedUser.idDocument ? (
                  <div className="relative rounded-xl overflow-hidden border border-gray-700 bg-gray-800">
                    ...
                    <a
                      ...
                      className="absolute top-2 right-2 rounded-lg bg-gray-900/80 border border-gray-700 p-1.5 hover:bg-gray-800 transition-colors"
                    >
                      <ExternalLink className="h-3.5 w-3.5 text-gray-300" />
                    </a>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2 py-8 rounded-xl border border-gray-700 bg-gray-800">
                    <ImageOff className="h-7 w-7 text-gray-600" />
      ...
      {verifiedUsers.length > 0 && (
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-5">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {verifiedUsers.slice(0, 12).map((u: any) => (
              <div key={u.id} className="flex items-center gap-3 rounded-lg bg-gray-800/50 border border-gray-700/50 px-3 py-2.5">
          ...
          {verifiedUsers.length > 12 && (
            <p className="mt-3 text-xs text-gray-500 text-center">+{verifiedUsers.length - 12} more verified hosts</p>
          )}
        </div>
      )}
      ...
      {showRejectModal && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-700 bg-gray-900 p-6 shadow-2xl">
            <h3 className="text-lg font-semibold text-white mb-1">Reject ID Verification</h3>
            <p className="text-sm text-gray-400 mb-4">
              Rejecting <span className="text-white font-medium">{selectedUser.firstName} {selectedUser.lastName}</span>'s ID document.
            </p>
            <textarea
              value={rejectReason}
              ...
              rows={3}
              className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-red-700 resize-none mb-4"
            />
            <div className="flex gap-3">
              <button
                onClick={() => { setShowRejectModal(false); setRejectReason(''); }}
                className="flex-1 rounded-lg border border-gray-700 px-4 py-2.5 text-sm text-gray-300 hover:bg-gray-800 transition-colors"
              >
                Cancel
              </button>
              <button
