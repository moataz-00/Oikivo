...
import { Plus, Pencil, Trash2, X, Shield, Sparkles, Zap } from 'lucide-react';
...
export default function AmenitiesPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-amenities'] }); setModal(null); },
  });
  ...
  const openCreate = () => {
    setForm({ name: '', nameAr: '', icon: '', category: 'essential', sortOrder: amenities.length });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">{amenities.length} amenities across 3 categories</p>
        </div>
      ...
      <div className="flex gap-2">
        {[{ key: 'all', label: 'All' }, ...Object.entries(CATEGORY_CONFIG).map(([key, { label }]) => ({ key, label }))].map(f => (
          <button key={f.key} onClick={() => setFilterCat(f.key)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filterCat === f.key ? 'bg-indigo-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-gray-200'}`}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-12 text-center">
          <p className="text-gray-400">No amenities found</p>
                    ...
                    <tbody>
                      {items.map(a => (
                        <tr key={a.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="p-3 text-gray-300" dir="rtl">{a.nameAr || '—'}</td>
                          <td className="p-3 text-gray-400">{a.sortOrder}</td>
                          <td className="p-3 text-right">
                            <div className="flex items-center justify-end gap-1">
                              <button onClick={() => openEdit(a)} className="p-1.5 rounded-lg hover:bg-gray-700 text-gray-400 hover:text-white transition-colors">
                                <Pencil className="h-3.5 w-3.5" />
                              </button>
                              <button onClick={() => { if (confirm(`Delete "${a.name}"?`)) deleteMut.mutate(a.id); }} className="p-1.5 rounded-lg hover:bg-red-900/30 text-gray-400 hover:text-red-400 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold text-white">{modal.mode === 'create' ? 'New Amenity' : 'Edit Amenity'}</h3>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="col-span-3">
                  ...
                  <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Amenity name" className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm mt-1" />
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-500">Name (AR)</label>
                <input value={form.nameAr} onChange={e => setForm(f => ({ ...f, nameAr: e.target.value }))} placeholder="اسم المرفق" dir="rtl" className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm mt-1" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500">Category</label>
                  <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value as Amenity['category'] }))} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm mt-1">
