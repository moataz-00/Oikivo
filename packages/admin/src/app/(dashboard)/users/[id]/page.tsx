...
export default function UserDetailPage() {
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-user', userId] }); qc.invalidateQueries({ queryKey: ['admin-users'] }); setEditing(false); toast.success('User updated'); },
    onError: (err: any) => toast.error(err?.response?.data?.message ?? 'Failed to update user'),
  });

  const deleteMut = useMutation({
    mutationFn: () => adminApi.deleteUser(userId),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-users'] }); router.push('/users'); },
  ...
  const toggleActiveMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-user', userId] }); qc.invalidateQueries({ queryKey: ['admin-users'] }); toast.success('Status toggled'); },
