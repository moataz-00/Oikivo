  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">InstaPay Refunds Pending</h1>
        ...
        <button
          onClick={() => refetch()}
          className="flex items-center gap-2 rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-gray-300 hover:text-white hover:border-gray-500 transition-colors"
        >
      ...
      {!isLoading && !isError && bookings.length === 0 && (
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">No pending InstaPay refunds at the moment.</p>
        </div>
      )}
      ...
      {!isLoading && !isError && bookings.length > 0 && (
        <div className="rounded-xl border border-gray-800 overflow-hidden">
          {/* Summary badge */}
          <div className="flex items-center gap-3 border-b border-gray-800 bg-gray-900 px-5 py-3">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900/60 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
            ...
            <tbody className="divide-y divide-gray-800">
              {bookings.map((b: any) => (
                <tr key={b.id} className="bg-gray-900 hover:bg-gray-800/60 transition-colors">
                  {/* Booking ID */}
                  <td className="px-5 py-4 font-mono text-xs text-gray-300">
                    <span className="rounded bg-gray-800 px-2 py-1 text-white">#{b.id}</span>
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-300 leading-tight max-w-[180px] truncate">
                        {b.property?.title ?? '—'}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4 text-gray-400 text-xs">
                    ...
                    {b.cancelledBy && (
                      <span className="ml-1 rounded bg-gray-800 px-1.5 py-0.5 text-gray-500">
                        by {b.cancelledBy}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-5 py-4 text-xs text-gray-400 max-w-[180px]">
                    {b.paymentReference && (
                      <p className="font-mono text-gray-300 truncate">{b.paymentReference}</p>
                    )}
                    {b.paymentNote && (
