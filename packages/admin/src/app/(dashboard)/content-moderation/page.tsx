            ...
            return (
              ...
              >
                ...
                <div className="relative h-40 bg-gray-100 dark:bg-gray-800">
                  {coverImage ? (
                    <img
                      src={getUploadUrl(typeof coverImage === 'string' ? coverImage : coverImage.url ?? coverImage)}
                      alt={p.title}
                  ...
                  {(p.photos?.length ?? 0) > 1 && (
                    <span className="absolute top-2 right-2 rounded-full bg-black/60 px-2 py-0.5 text-xs text-gray-600 dark:text-gray-300">
                ...
                {previewId === p.id && (
                  <div className="border-t border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-950 p-4 text-sm space-y-3">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Full Description</p>
                    <p className="text-gray-600 dark:text-gray-300 text-xs leading-relaxed line-clamp-6">
                      {p.description || <span className="text-gray-600 italic">No description provided</span>}
                    </p>
                    {(p.amenities?.length ?? 0) > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Amenities</p>
                        <div className="flex flex-wrap gap-1">
                          {p.amenities.map((a: any) => (
                            <span key={a.id ?? a} className="rounded bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 text-xs text-gray-600 dark:text-gray-300">
                              {a.name ?? a}
                            </span>
                          ))}
                        </div>
                    ...
                    {(p.photos?.length ?? 0) > 1 && (
                      <div className="flex gap-1.5 overflow-x-auto pb-1">
                        {p.photos.slice(1).map((img: any, i: number) => (
                          <img
                            key={i}
                            src={getUploadUrl(typeof img === 'string' ? img : img.url ?? img)}
                            alt=""
