...
export default function ExperienceBookingsAdminPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Experience Bookings</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage all experience bookings and confirm payments</p>
      </div>
        ...
        <form onSubmit={handleSearch} className="ml-auto flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 pl-9 pr-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-64"
            />
          </div>
                ...
                : items.map((b: any) => (
                  <tr key={b.id} onClick={() => router.push(`/experience-bookings/${b.id}`)} className="hover:bg-gray-800/50 transition-colors cursor-pointer">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-gray-900 dark:text-white font-medium">{b.guest?.firstName} {b.guest?.lastName}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{b.guest?.email}</p>
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-gray-900 dark:text-white font-medium truncate max-w-[180px]">{b.experience?.title}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Host: {b.host?.firstName} {b.host?.lastName}</p>
                    </td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                      <p>{b.bookingDate}</p>
                      <p className="text-xs text-gray-500">{b.startTime}</p>
                    </td>
                    <td className="px-4 py-3 text-center text-gray-600 dark:text-gray-300">{b.guestsCount}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                    ...
                    <td className="px-4 py-3">
                      <span className={cn('inline-flex rounded-full px-2 py-0.5 text-xs font-medium', STATUS_COLORS[b.status] ?? 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400')}>
                        {b.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={cn('inline-flex rounded-full px-2 py-0.5 text-xs font-medium', PAYMENT_COLORS[b.paymentStatus] ?? 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400')}>
                        {b.paymentStatus}
                      </span>
                      {b.paymentReference && (
          ...
          <div className="flex items-center justify-between border-t border-gray-200 dark:border-gray-800 px-4 py-3">
            ...
            <div className="flex gap-1">
              <button
                ...
                disabled={page === 1}
                className="rounded-lg p-1.5 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-40 transition-colors"
              >
