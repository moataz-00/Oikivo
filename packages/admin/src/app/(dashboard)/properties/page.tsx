...
import { Search, ChevronLeft, ChevronRight, Archive, Eye, FileEdit, CheckSquare, Square, X, MapPin, Users, Bed, Bath, Star, DollarSign, Calendar, Home } from 'lucide-react';
...
function PropertyDetailModal({ propertyId, onClose }: { propertyId: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-gray-700 bg-gray-900 shadow-2xl"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-5 px-6 pb-6">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={cn('shrink-0 rounded-full px-3 py-1 text-xs font-medium', STATUS_COLORS[p.status] ?? 'bg-gray-700 text-gray-400')}>
                  {p.status}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={item.label} className="rounded-xl bg-gray-800/60 border border-gray-700 p-3 text-center">
              ...
              {p.description && (
                <div>
                  ...
                  <p className="text-sm text-gray-300 whitespace-pre-line leading-relaxed">{p.description}</p>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.amenities.map((a: any) => (
                      <span key={a.id ?? a.name} className="rounded-full bg-gray-800 border border-gray-700 px-3 py-1 text-xs text-gray-300">
                        {a.name ?? a.title ?? a}
                      </span>
                    ))}
                  </div>
              ...
              {p.houseRules && (
                <div>
                  ...
                  <p className="text-sm text-gray-300 whitespace-pre-line">{p.houseRules}</p>
                </div>
              )}
              ...
              {p.cancellationPolicy && (
                <div>
                  ...
                  <p className="text-sm text-gray-300 capitalize">{p.cancellationPolicy}</p>
                </div>
              )}
            </div>
          </div>
        ) : (
          <div className="p-6 text-center text-gray-400 text-sm">Property not found.</div>
        )}
      </div>
    </div>
  );
                    ...
                    return (
                      <tr key={p.id} className={cn('transition-colors cursor-pointer', selected.includes(p.id) ? 'bg-indigo-900/20' : 'hover:bg-gray-800/50')} onClick={() => setDetailId(p.id)}>
                        <td className="px-4 py-3">
                          <button onClick={(e) => { e.stopPropagation(); toggleOne(p.id); }} className="text-gray-400 hover:text-white transition-colors">
