...
export default function PropertyDetailPage() {
  ...
  const invalidId = isNaN(propertyId);
  ...
  if (invalidId) return <div className="text-gray-500 dark:text-gray-400 text-center py-20">Invalid property ID</div>;
  ...
  if (!property) return <div className="text-gray-500 dark:text-gray-400 text-center py-20">Property not found</div>;

  const isFeatured = (property as any).isFeatured as boolean | undefined;
  ...
  const statusColor = (s: string) => s === 'published' ? 'bg-emerald-900/40 text-emerald-400' : s === 'draft' ? 'bg-yellow-900/40 text-yellow-400' : s === 'archived' ? 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400' : 'bg-blue-900/40 text-blue-400';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white truncate">{property.title}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Views', value: property.viewCount ?? 0, icon: Eye, color: 'text-gray-500 dark:text-gray-400' },
        ].map(s => (
      ...
      {(property.photos?.length ?? 0) > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Photos ({property.photos.length})</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {property.photos.map((p: any, i: number) => (
              <img key={p.id ?? i} src={getUploadUrl(p.url)} alt="" className="w-full h-28 object-cover rounded-lg border border-gray-300 dark:border-gray-700" />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 space-y-4">
          ...
          {editing ? (
            ...
            <>
              ...
              <div className="flex flex-wrap gap-4 pt-2 text-sm text-gray-600 dark:text-gray-300">
            ...
            <div className="pt-3">
              ...
              <ul className="space-y-1">
                {property.houseRules.map((r: any, i: number) => {
                  const ruleText = typeof r === 'string' ? r : (r.rule ?? r.ruleEn ?? r.text ?? '');
                  return <li key={i} className="text-sm text-gray-600 dark:text-gray-300">• {ruleText}</li>;
                })}
              </ul>
            </div>
          )}
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 space-y-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Status Actions</h2>
            {['published', 'draft', 'archived'].map(s => (
              <button key={s} onClick={() => statusMut.mutate(s)} disabled={property.status === s || statusMut.isPending}
                className={`w-full px-4 py-2.5 rounded-lg text-sm font-medium transition-colors capitalize disabled:opacity-30 ${property.status === s ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
          ...
          {/* Featured & Commission */}
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6 space-y-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Promotion</h2>
            <button onClick={() => featuredMut.mutate()} disabled={featuredMut.isPending} className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 ${isFeatured ? 'bg-amber-600 hover:bg-amber-500 text-white' : 'bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
            ...
            <div>
              ...
              {commissionEdit ? (
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setCommissionEdit(false)} className="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-900 dark:text-white text-xs rounded">×</button>
                </div>
              ) : (
                <p className="text-sm text-gray-900 dark:text-white font-medium">{Number(property.serviceFeePercent ?? 14).toFixed(1)}%</p>
              )}
            </div>
          </div>
          ...
          {property.host && (
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Host</h2>
              <Link href={`/users/${property.host.id}`} className="flex items-center gap-3 hover:bg-gray-800/50 rounded-lg p-2 -m-2 transition-colors">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-white">{property.host.firstName} {property.host.lastName}</p>
      ...
      {(property.reviews?.length ?? 0) > 0 && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Reviews ({property.reviews.length})</h2>
          <div className="space-y-3">
            {property.reviews.slice(0, 10).map((r: any) => (
              <div key={r.id} className="border-b border-gray-200/50 dark:border-gray-800/50 pb-3">
    ...
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Admin Notes / Change Requests</h2>
        </div>
        {!editing && (
