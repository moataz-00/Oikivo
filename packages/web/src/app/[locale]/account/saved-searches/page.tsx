...
export default function SavedSearchesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('searchDeleted') ?? 'Search deleted');
    },
    onError: () => toast.error(t('deleteError') ?? 'Could not delete search'),
  });
  ...
  const toggleAlertMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['saved-searches'] });
      toast.success(t('alertToggled') ?? 'Alert updated');
    },
  });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-10">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-neutral-900">{t('savedSearches') ?? 'Saved Searches'}</h1>
          <p className="text-sm text-neutral-500">{t('savedSearchesDesc') ?? 'Manage your saved searches and price alerts'}</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <h2 className="text-lg font-semibold text-neutral-900">{t('noSavedSearches') ?? 'No saved searches yet'}</h2>
          <p className="text-sm text-neutral-500 mt-1">{t('noSavedSearchesDesc') ?? 'Save a search from the search page to get notified about new listings.'}</p>
                ...
                {search.filters && (
                  <p className="text-xs text-neutral-500 mt-0.5 truncate">
                    {[
                      ...
                      search.filters.guests && `${search.filters.guests} guests`,
